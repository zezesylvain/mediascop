...
SELECT;
    ...
    public static function makeListeDate(): array
    {
        $dates = [];
...
STR;
    ...
    public static function echoValue($column, $value, $table = NULL) {
        if(!self::is_foreignKey($column)):
            if($column == "parent"):
                ...
                return self::getChampTable($table, $value) ;
            endif;
            if($column == "code" && $table == "icons") :
                return "<i class=\"fa fa-$value\"></i>";
            endif;
            return $value;
        endif;
        if($column == "icon"):
            $text = self::getChampTable("icons", $value, "code");
            return "<i class=\"fa fa-$text\"></i>" ;
        endif;
        return self::getChampTable($column."s", $value);
    }

    public static function getTableFieldDefault() {
        if (!Session::has("TableFieldDefault")):
            $tabFieldDefault = [];
            $table = DbTablesHelper::dbTable('DBTBL_TABLEDEFAULTFIELDS');
            $table = self::getTableName($table);
            $re = self::arraySqlResult("SELECT * FROM $table");
            foreach ($re AS $v):
                $tabFieldDefault[$v["field"]] = $v;
            endforeach;
            Session::put("TableFieldDefault", $tabFieldDefault);
        endif;
    }

    public static function diff_date($date1, $date2, $format = "%y-%m-%d"):string{
    return date_diff(date_create($date1), date_create($date2))->format($format);
    }

    public static function getTableListeField() {
        if (!Session::has("TableListeField")):
            $tab = [];
            $tables = self::getDatabaseTable ();
            foreach ($tables as $r):
                $listeDesChamps = self::getListeDesChamps ($r);
                $tab[$r] = $listeDesChamps;
            endforeach;
            Session::put("TableListeField", $tab);
        endif;
        return Session::get ("TableListeField");
    }

    public static function getTabExtension(string $str):array {
        return Config::get("constantes.$str");
    }

    public static function icone(string $ext){
        switch ($ext):
            case 'docx' :
            case 'doc' :
                $n = '-word' ;
    ...
    /**
     * @param int $annonceurID
     * @return string
     */
    public static function getIconAnnonceur(int $annonceurID,string $directorie = null): string
    {
        $dir = $directorie === null ? 'upload'.DIRECTORY_SEPARATOR.'annonceurs'.DIRECTORY_SEPARATOR.$annonceurID.DIRECTORY_SEPARATOR : 'upload'.DIRECTORY_SEPARATOR.'annonceurs'.DIRECTORY_SEPARATOR.$annonceurID.DIRECTORY_SEPARATOR.$directorie.DIRECTORY_SEPARATOR;
        $directorie = public_path($dir);
        $uri = '';
        if (is_dir($directorie)):
            $files = glob($directorie.'*');
            $cp = count($files);
            if ($cp):
                $file = $cp === 1 ? $files[0] : $files[1];
                if (is_file($file)):
                    $info = pathinfo($file);
                    if ($info['filename'] === 'logo'):
                        $uri .= asset ($dir.$info['basename']);
                    endif;
                endif;
            endif;
        endif;
