...
class ReportingController extends AdminController
{
    ...
    public static function built($donnees){
        ...
        $data = compact('lesDonnees', 'secteurs','natures','route','medias', 'donnees','nbrsecteur',
                    'nbrnature','nbrmedia','date_fin','date_debut', 'PlanMediaData', 'hauteur', 'hauteur2') ;
        return view('clients.index', $data) ;
    }
    public static function planMediaQuery($cond){
        $tblReportings = DbTablesHelper::dbTable("DBTBL_REPORTINGS",'db') ;
        $query1 =  "
                    SELECT 
                        `annonceur`, `media`, `support`, `date` , COUNT(*) AS insertion, SUM(`tarif`) AS investissement
                    FROM 
                        `$tblReportings` 
                    WHERE 
                        $cond
                    GROUP BY 
                        `secteur`, `annonceur`, `media`, `support`, `date` ";
        $query2 = "SELECT 
                        DISTINCT(`date`)
                    FROM 
                        `$tblReportings` 
    ...
    public static function makeReportingData(array $donnees):array{
        ...
        foreach($donnees AS $row):
            ...
            $tarif = $media == "AFFICHAGEZ" ? $row['investissement'] : $row['tarif'] ;
            $insertion = $media == "AFFICHAGEZ" ? $row['nombre'] : 1 ;
        ...
        return compact(['lesDate' , 'dateMois' , 'detailDesCampagnes', 'investParAnnonceurEtParMedia',
                ...
                'investParMedia', 'investParMediaEtParSupport', 'investParMediaEtParAnnonceur',
    ...
    public static function getAnnonceurInfo(array $annonceurs):array{
        ...
        $d = FunctionController::arraySqlResult("SELECT * FROM $dbtbl WHERE id IN ($join)") ;
        foreach($d AS $r):
    ...
    public static function getSpeednews($elt) {
        ...
        $cm = " media IN (".join(',', $elt['media']).")" ;
        $cs = " support IN (".join(',', $elt['support']).")" ;
        $table = DbTablesHelper::dbTable('DBTBL_SPEEDNEWS','db') ;
        $sql = "SELECT * FROM $table WHERE $cc AND $cm AND $cs";
    ...
    public static function getDocCampagne($elt) {
        ...
        $sql = "SELECT * FROM $table WHERE $cc AND $cm";
        $result = FunctionController::arraySqlResult($sql);
        foreach ($result as $value):
    ...
    public static function makeSpeednewsTable($elt, $detailDesCampagnes){
        ...
        foreach ($lesSpeednews as $leSpeednews):
            ...
            //dd($cid, $detailDesCampagnes[$cid]['support'], $mediaID) ;
