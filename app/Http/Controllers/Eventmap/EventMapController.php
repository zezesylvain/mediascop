...
namespace App\Http\Controllers\Eventmap;
...
class EventMapController extends Controller
{
    ...
    public function accueil(Request $request){
        ...
        //$this->nettoyerSession ("EventMapSelection");
        ...
        if(Session::has('EventMapSelection')):
            $selection = Session::get('EventMapSelection.selection');
            $listeDesMap = Session::get('EventMapSelection.listeDesMap');
        else:
            $datas = [
                'secteur' => [1],
        ...
        return view ("eventmap.tplclient", compact ('dateDebut', 'dateFin', 'secteurs','nbrsecteur','villes','listeDesMap','selection','formats','nbrformat'));
    }
    ...
    public function gestionLocalite()
    {
        ...
        $tableau = ModuleController::makeTable (DbTablesHelper::dbTable ('DBTBL_LOCALITES','db'),"",['routeUpdate' => 'eventmap.updateLocalite']);
        return view ("eventmap.localites.form", compact ('localites','tableau'));
    }
    public function updateLocalite(string $table, int $id)
    {
        ...
        $tableau = ModuleController::makeTable (DbTablesHelper::dbTable ('DBTBL_LOCALITES','db'),"",['routeUpdate' => 'eventmap.updateLocalite']);
        return view ("eventmap.localites.formUpdate", compact ('localites','tableau','localite','grandPere'));
    ...
    public function getCommune(Request $request){
        ...
        if (!is_null ($request->localite)):
            ...
            //  return view ("eventmap.panneaux.inputCommune",compact ('communes','send'))->render ();
        endif;
    ...
    public function bbordmapSupport(Request $request){
        ...
        if (count ($tabRegie)):
            ...
            return view ("eventmap.panneaux.listePanneaux",compact ('panneaux'))->render ();
        endif;
    }
    ...
    public function bbordmapComm(Request $request){
        ...
        if (array_key_exists ('secteur',$request->all())){
            ...
            return view ("eventmap.panneaux.listeAnnonceurs",compact ('annonceurs'))->render ();
            //endif;
        }
        if (array_key_exists ('annonceur',$request->all ())){
            ...
            if (count ($tabAnnonceur)):
                ...
                return view ("eventmap.panneaux.listeCampagnes",compact ('campagnes'))->render ();
            endif;
        }
    ...
    public function bbordmapLocalite(Request $request){
        ...
        if (array_key_exists ('ville',$request->all ())){
            ...
            if (count ($tabVille)):
                ...
                return view ("eventmap.panneaux.listeCommunes",compact ('communes'))->render ();
            endif;
        }
    }
//*/
     ...
     */
    public function trouverEvenements(Request $request): RedirectResponse
    {
        ...
        if (!$validate):
            ...
            $request->session()->forget('EventMapSelection');
            $datas = $request->all ();
        ...
        if (!empty($datas)):
            ...
            if ($condition !== ''):
                ...
                if (!empty($donnees)):
                    ...
                    $this->listeDesEvenements($listeDesMap);
                else:
     ...
     * @throws \Throwable
     */
    public function listeDesEvenements(array $listeDesMap)
    {
        $selection = AdminController::afficherLaSelection($this->recherche);
        Session::put('EventMapSelection', [
    ...
    public function addLieuLocalite()
    {
        $localites = FunctionController::arraySqlResult ("SELECT * FROM ".DbTablesHelper::dbTable ('DBTBL_LOCALITES','db')." WHERE parent = 0 ORDER BY name ASC");
        $tableau = ModuleController::makeTable (DbTablesHelper::dbTable ('DBTBL_POINTS','db'),"",[]);
        return view ("eventmap.localites.form", compact ('localites','tableau'));

    }

    public function storeLieuLocalites(Request $request)
    {
        $this->validate($request,[
            ...
            'name.required' => 'Le lieu de la localité est obligatoire!',
            'localite.required' => 'La localité est obligatoire!',
            'latitude.required' => 'La latitude est obligatoire!',
            'longitude.required' => 'La longitude est obligatoire!',
        ]);
        $data = $request->all();
        $data['localite_id'] = $request->input('localite');
        unset($data['_token'],$data['localite']);
        $ins = DB::table(DbTablesHelper::dbTablePrefixeOff('DBTBL_POINTS','db'))
            ->insertGetId($data);
        if ($ins):
            $request->session()->flash('success','Le lieu a été sauvegardé avec succès!');
        else:
            $request->session()->flash('echec','La sauvegarde du lieu a échoué!');
        endif;
        return back();
    }

    public function getLieuLocalite(Request $request)
    {
        $lieuLocalites = Point::where('localite_id',$request->input('localite'))->get()->toArray();
        //dd($lieuLocalites);
        return $lieuLocalites;
    }

    
    public function makeDonnees(array $result)
    {
        $keys = ['localite', 'point_id', 'annonceur'];
        $investPar = [];
        $insertPar = [];
        $dataRange = [];
        $dataRangePoint = [];
        ...
        foreach ($result as $row) :
            foreach ($keys as $k) :
                $rk = $row[$k];
                $var = "les" . ucfirst($k) . "s";
                if (!array_key_exists($rk, $investPar[$k])) :
                    $investPar[$k][$rk] = $row['investissement'];
                    $insertPar[$k][$rk] = $row['nombre'];
                else :
                    $investPar[$k][$rk] += $row['investissement'];
                    $insertPar[$k][$rk] += $row['nombre'];
                endif;
            ...
            endforeach;
            //dump($row);
            $this->makeRow($row, 'localite', $dataRange);
            $p = $row['point_id'];
            if ($p) :
                $this->makeRow($row, 'point_id', $dataRangePoint);
            else :
                $this->makeRow($row, 'localite', $dataRangeLocalite);
            endif;
        ...
        endforeach;
        return compact('lesCampagnes' ,'investPar' , 'insertPar', 'dataRange', 'dataRangePoint', 'dataRangeLocalite', 'lesAnnonceurs', 'lesPoint_ids', 'lesLocalites');
    }
    public function makeRow($row, $tp, &$donnees){
        $localite = $row['localite'];
        $type = $row[$tp];
        $ligne = $donnees[$localite] ?? [];
        if (!array_key_exists($type, $ligne)) :
            $ligne[$type][$row['annonceur']] = [
                'campagne'  => [$row['campagnetitle']],
                'nb'        => $row['nombre'],
                'invest'    => $row['investissement']
            ];
            $ligne[$type][$row['annonceur']]['logo'] = str_replace('\\', '/', FunctionController::getIconAnnonceur($row['annonceur'], 'Map'));
            $ligne[$type][$row['annonceur']]['couleur'] = FunctionController::getChampTable(DbTablesHelper::dbTable('DBTBL_ANNONCEURS', 'db'), $row['annonceur'], 'couleur');
            $ligne[$type][$row['annonceur']]['date'] = $row['date'];
        elseif (!array_key_exists($row['annonceur'], $ligne[$type])) :
            $ligne[$type][$row['annonceur']] = [
                'campagne'  => [$row['campagnetitle']],
                'nb'        => $row['nombre'],
                'invest'    => $row['investissement']
            ...
            if (!in_array($row['campagnetitle'], $ligne[$type][$row['annonceur']]['campagne'])) :
                $ligne[$type][$row['annonceur']]['campagne'][] = $row['campagnetitle'];
            endif;
            $ligne[$type][$row['annonceur']]['nb'] += $row['nombre'];
            $ligne[$type][$row['annonceur']]['invest'] = $row['investissement'];
        endif;
        $donnees[$localite] = $ligne;
    }
     ...
     */
     //*
    public function makeMapDatasV(array $result): array
    {
        $investParLocalite = [];
        $investParAnnonceur = [];
        $insertParLocalite = [];
        ...
        foreach($result as $row) :
            ...
            if($p):
                if(!array_key_exists($p, $dataRangePoint)):
                $dataRangePoint[$p][$row['annonceur']] = [
                    'campagne'  => [$row['campagnetitle']],
                    'nb'        => $row['nombre'],
                    'invest'    => $row['investissement']
                ];
             elseif(!array_key_exists($row['annonceur'], $dataRangePoint[$p])):
                $dataRangePoint[$p][$row['annonceur']] = [
                    'campagne'  => [$row['campagnetitle']],
                    'nb'        => $row['nombre'],
                    'invest'    => $row['investissement']
                ...
                if(!in_array($row['campagnetitle'], $dataRangePoint[$p][$row['annonceur']]['campagne'])):
                   $dataRangePoint[$p][$row['annonceur']]['campagne'][] = $row['campagnetitle'];
                endif;
                    $dataRangePoint[$p][$row['annonceur']]['nb'] += $row['nombre'];
                    $dataRangePoint[$p][$row['annonceur']]['invest'] = $row['investissement'];
                endif;
            else:
                if(!array_key_exists($row['localite'], $dataRangeLocalite)):
                    $dataRangeLocalite[$row['localite']][$row['annonceur']] = [
                        'campagne'  => [$row['campagnetitle']],
                        'nb'        => $row['nombre'],
                        'invest'    => $row['investissement']
                    ];
                 elseif(!array_key_exists($row['annonceur'], $dataRangeLocalite[$row['localite']])):
                    $dataRangeLocalite[$row['localite']][$row['annonceur']] = [
                        'campagne'  => [$row['campagnetitle']],
                        'nb'        => $row['nombre'],
                        'invest'    => $row['investissement']
                    ...
                    if(!in_array($row['campagnetitle'], $dataRangeLocalite[$row['localite']][$row['annonceur']]['campagne'])):
                        $dataRangeLocalite[$row['localite']][$row['annonceur']]['campagne'][] = $row['campagnetitle'];
                    endif;
                    $dataRangeLocalite[$row['localite']][$row['annonceur']]['nb'] += $row['nombre'];
                    $dataRangeLocalite[$row['localite']][$row['annonceur']]['invest'] = $row['investissement'];
                endif;
            endif;
     ...
     * @return string[]
     */
    public function makeMapDatasBon(array $result): array
    {
        $donnees = $this->makeDonnees($result);
        extract($donnees);
        $investParLocalite = $investPar['localite'];
        $investParAnnonceur = $investPar['annonceur'];
        $insertParLocalite = $insertPar['localite'];
        $insertParAnnonceur = $insertPar['annonceur'];
        $listeDesPoints = [];
        $listeDesPointsNew = '';
        $virg = '';
        $Bigdata = '';
        $tableauDesMapNew = [];
        $lesLocalitesB = self::arrayByKey(Localite::whereIn('id', $lesLocalites)->get()->toArray());
        $lesAnnonceursB = self::arrayByKey(Annonceur::whereIn('id', $lesAnnonceurs)->get()->toArray());
        $lesPointsB = self::arrayByKey(Point::with('localite')->whereIn('id', $lesPoint_ids)->get()->toArray());
        //dd($lesLocalitesB, $lesPointsB, $lesAnnonceursB);

        $lesCouleurs = [];
        foreach($lesAnnonceursB AS $loc):
            $lesCouleurs[$loc['name']] = Str::startsWith($loc['couleur'], "#") ? $loc['couleur'] : "#".$loc['couleur'] ;
            if(array_key_exists($loc['id'], $lesCampagnes)):
                $lesCampagnes[$loc['id']]['name'] = $loc['name'];
                $lesCampagnes[$loc['id']]['slug'] = $loc['slug'] ?? str_split($loc['name'], 2)[0];
            endif;
        endforeach;
        $lesTabCamp = self::makeAbbreviationCampagne($lesCampagnes);
        ...
        $insertParAnnonceurBB = [];
        foreach($investParLocalite AS $k => $v):
            $investParLocaliteBB[$lesLocalitesB[$k]['name']] = intval($v) ;
            $insertParLocaliteBB[$lesLocalitesB[$k]['name']] = intval($insertParLocalite[$k]) ;
        endforeach;
        foreach($insertParAnnonceur AS $k => $v):
            $investParAnnonceurBB[$lesAnnonceursB[$k]['name']] = intval($investParAnnonceur[$k]) ;
            $insertParAnnonceurBB[$lesAnnonceursB[$k]['name']] = intval($v) ;
        endforeach;
        $donneesAnnonceurs = $lesAnnonceursB;
        //$points = [];
        $j = 0;
        $k = $j;
        $points = [];
        //dd($dataRange, $dataRangePoint, $dataRangeLocalite, $lesLocalitesB, $lesPointsB);
        foreach($lesLocalitesB AS $local):
            $lat = $local['latitude'];
            $long = $local['longitude'];
            $nomLocalite = $local['name'];
            $donnees = $dataRange[$local['id']][$local['id']];
            if (!array_key_exists($local['id'],$points)):
                $points = self::makeMapPoint($lat, $long, count($donnees),$local['id']);
            endif;
            $i = 0;
    ...
    public function makeMapDatas(array $result): array
    {
        ...
        $k = $j;
        $points = [];
        //dd($lesLocalitesB, $dataRangePoint, $dataRange, $lesPointsB, $dataRangeLocalite);
        //dd($dataRangePoint, $lesPointsB);
        foreach($lesLocalitesB AS $lid => $laLocalite){
            if(array_key_exists($lid, $dataRangePoint)):
                $lesKeys = array_keys($dataRangePoint[$lid]) ;
                $nomLocalite = $laLocalite['name'];
                foreach($lesKeys AS $pid):
                    $lat = $lesPointsB[$pid]['latitude'];
                    $long = $lesPointsB[$pid]['longitude'];
                    $nomLieu = $lesPointsB[$pid]['name'];
                    $donnees = $dataRangePoint[$lid][$pid];
                    if (!array_key_exists($pid,$points)):
                        $points = self::makeMapPoint($lat, $long, count($donnees), $pid);
                    endif;
                    $i = 0;
                    $anTb = [];
                    foreach($donnees AS $annonceur_id => $tab):
                        $img = $tab['logo'];
                        $couleur = $tab['couleur'];
                        $latitude = $points[$pid][$i]['lat'];
                        $longitude = $points[$pid][$i]['lon'];
                        $nombrePanneaux = $tab['nb'];
                        $annonceur = $donneesAnnonceurs[$annonceur_id]['raisonsociale'];
                        $campagneTitle = "";
                        $investissement = $tab['invest'];
                        ...
                        $tableauDesMapNew[$lid]["lieu$pid"][$annonceur_id] = [
                            'Date' => FunctionController::date2Fr($tab['date']),
                            'titre' => $campagneTitlesListe,
                            'annonceur' => $annonceur,
                            'Lieu'      => $nomLieu,
        ...
        foreach($lesLocalitesB AS $lid => $laLocalite){
            if(array_key_exists($lid, $dataRangeLocalite)):
                $lesKeys = array_keys($dataRangeLocalite[$lid]);
                $nomLocalite = $laLocalite['name'];
                foreach($lesKeys AS $pid):
                    $lat = $lesLocalitesB[$pid]['latitude'];
                    $long = $lesLocalitesB[$pid]['longitude'];
                    $nomLieu = $lesLocalitesB[$pid]['name'];
                    $donnees = $dataRangeLocalite[$lid][$pid];
                    if (!array_key_exists($pid,$points)):
                        $points = self::makeMapPoint($lat, $long, count($donnees), $pid);
                    endif;
                    $i = 0;
                    ...
                    foreach($donnees AS $annonceur_id => $tab):
                        ...
                        $tableauDesMapNew[$lid]["localite$pid"][$annonceur_id] = [
                            'Date' => FunctionController::date2Fr($tab['date']),
                            'titre' => $campagneTitlesListe,
                            'annonceur' => $annonceur,
                            'Lieu'      => $nomLieu,
    ...
    public function makeGoogleMapData( array $type, array $data, array $donneesAnnonceurs, $lieu = true, $j = 0){
        $k = $j;
        $points = [];
        $Bigdata = '';
        foreach($type AS $t):
            ...
            $nomLocalite = $t['name'];
            $donnees = $data[$t['id']];
            if (!array_key_exists($t['id'],$points)):
                $points = self::makeMapPoint($lat, $long, count($donnees),$t['id']);
            endif;
            $i = 0;
            $anTb = [];
            foreach($donnees AS $annonceur_id => $tab):
                $img = $tab['logo'];
                $couleur = $tab['couleur'];
                $latitude = $points[$t['id']][$i]['lat'];
                $longitude = $points[$t['id']][$i]['lon'];
                $nombrePanneaux = $tab['nb'];
                $annonceur = $donneesAnnonceurs[$annonceur_id]['raisonsociale'];
                $campagneTitle = "";
                $investissement = $tab['invest'];
