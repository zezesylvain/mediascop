...
class NewReportingController extends AdminController
{
    ...
    public function home(){
        ...
        $q = self::planMediaQuery($date_debut, $date_fin) ;
        ...
        $PlanMediaData['date'] =  FunctionController::arraySqlResult($q['queryDate']) ;
        $PlanMediaData['data'] =  FunctionController::arraySqlResult($q['queryData']) ;
        return view('clients.index', compact('secteurs','natures','route','medias',
                    'si_form_valide','menu_graphique','menu_recherche','nbrsecteur',
                    'nbrnature','nbrmedia','date_fin','date_debut', 'PlanMediaData'));

    }
    public static function planMediaQuery($dateDebut, $datFin, $cond = ""){
        $tblReportings = DbTablesHelper::dbTable("DBTBL_REPORTINGS",'db') ;
        $query1 =  "
                    SELECT 
                        `annonceur`, `media`, `support`, `date` , COUNT(*) AS nb
                    FROM 
                        `$tblReportings` 
                    WHERE 
                        `date` BETWEEN '$dateDebut' AND '$datFin' $cond
                    GROUP BY 
                        `annonceur`, `media`, `support`, `date` ";
        $query2 = "SELECT 
                        DISTINCT(`date`)
                    FROM 
                        `$tblReportings` 
                    WHERE 
                        `date` BETWEEN '$date_debut' AND '$date_fin'  $cond
                    ORDER BY `date` ASC" ;
        return [
                    "queryDate" => $query2,
                    "queryData" => $query1
    ...
    public function formReport(){
       ...
       // $speednewTbody = $mediascop->makeSpeednewsTable();
        ...
        $voixParAnnonceurMedia = [
            ...
            [
                "name" => "MTN CI",
                "data" => [ 130, 250, 140, 200, 150, 50],
                "color"=> "#303030"
            ],
            [
                "name" => "MOOV CI",
                "data" => [10, 120, 90, 250, 300, 400],
                "color"=> "#ff0033"
            ],
            [
                "name" => "YOOMEE",
                "data" => [100, 110, 90, 50, 150, 320],
                "color"=> "#11dd33"
            ]
        ] ;

        $x = ['TV', 'Radio', 'Presse', 'Internet', 'Mobile', 'Affichage'];
        $c3Data["stocked1"] = c3JsFunctionController::makeC3Data("stocked1","Part de voix par Annonceur et par MEDIA","Description",$voixParAnnonceurMedia,$x);
        //dd ($voixParAnnonceurMedia);
        return view('clients.index', compact('hauteur','hauteur2', 'ParSecteur','listDesCouleursDesAnnonceur','investParAnnonceur','investParCible','investParNature','investParOffretelecom','investParMedia','partDeVoixParMedia','investParMediaParAnnonceur','investParAnnonceurEtParMedia','selection','lesspeednews','si_form_valide','speednewTbody','listeCampagne','rapports','lestableaux','route','secteurs','natures','medias','menu_graphique','menu_recherche','nbrsecteur','nbrmedia','nbrnature','voixParAnnonceurMedia','x','c3Data'));
    }
//*/
    ...
    public function formReport(){
       ...
       // $speednewTbody = $mediascop->makeSpeednewsTable();
        ...
        $voixParAnnonceurMedia = $mediascop->transformData() ;
        $label = $mediascop->listMediaRange();
        //dd($label, $voixParAnnonceurMedia) ;
        $c3Data["stocked1"] = c3JsFunctionController::makeC3Data("Part de voix par Annonceur et par MEDIA","Description",$voixParAnnonceurMedia,'bar',true,$label);
        //dd ($voixParAnnonceurMedia);

        $stockage = [
            [
                "name" => "Orange CI",
                "data" => [30, 200, 100, 400, 150, 250, 300, 350, 250, 150, 300, 350, 150, 400, 150, 250, 300, 400, 150, 250, 300],
                "color"=> "#03a9f4"
            ],
            [
                "name" => "MTN CI",
                "data" => [ 130, 250, 140, 200, 150, 50, 100, 150, 200, 250, 300, 350, 400, 400, 150, 250, 300, 400, 150, 250, 300],
                "color"=> "#303030"
            ],
            [
                "name" => "MOOV CO OL",
                "data" => [10, 120, 90, 250, 300, 400, 360, 310, 280, 250, 200, 350, 380, 400, 150, 250, 300, 400, 150, 250, 300],
                "color"=> "#ff0033"
            ],
            [
                "name" => "AIRTEL",
                "data" => [100, 110, 90, 50, 150, 320, 400, 300, 150, 20, 100, 150, 250, 400, 150, 250, 300, 400, 150, 250, 300],
                "color"=> "#11dd33"
            ]
        ] ;
        $stockage1 = [
            [
                "name" => "TELECOM",
                "data" => [30, 200, 100, 400, 150, 250, 300, 350, 250],
                "color"=> "#03a9f4"
            ],
            [
                "name" => "ADMIN",
                "data" => [ 130, 250, 140, 200, 150, 50, 100, 150, 200],
                "color"=> "#303030"
            ],
            [
                "name" => "LAVERIE",
                "data" => [10, 120, 90, 250, 300, 400, 360, 310, 280],
                "color"=> "#ff0033"
            ],
            [
                "name" => "EDUCATION",
                "data" => [100, 110, 90, 50, 150, 320, 400, 300, 150],
                "color"=> "#11dd33"
            ]
        ] ;

        $c3Data['stocked0'] = c3JsFunctionController::makeC3Data ('Bar','Histogramme',$stockage,'bar',true);
        $c3Data['stocked2'] = c3JsFunctionController::makeC3Data ('Bar','Histogramme',$stockage1,'bar',false);
        $c3Data['stocked3'] = c3JsFunctionController::makeC3Data ('Bar','Courbes',$stockage,'spline');
        $c3Data['stocked4'] = c3JsFunctionController::makeC3Data ('area-spline','Courbe surface',$stockage1,'bar',true);
    ...
    public function illustrationCampagne($cid, $media, $use_link = true){
        $ret = "" ;
        $return = "" ;
