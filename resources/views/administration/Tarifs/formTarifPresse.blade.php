<div class="row">
    ...
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
        ...
        <select name="presse_page" id="emplacement" data-placeholder="Choisissez un emplacement" class="chosen-select form-control" tabindex="-1" required>
            <option value="">Choisir un emplacement</option>
            @foreach($pressePages as $row)
                <option value="{{$row['id']}}">{{$row['name']}}</option>
            @endforeach
        </select>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
        ...
        <select name="presse_calibre" id="calibre" data-placeholder="Choisissez un support" class="chosen-select form-control" tabindex="-1" required>
            <option value="">Choisir un calibre</option>
            @foreach($calibres as $row)
                <option value="{{$row['id']}}">C{{$row['name']}}</option>
            @endforeach
        </select>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
