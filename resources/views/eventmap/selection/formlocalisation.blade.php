                ...
                <div class="col-sm-6">
                    ...
                    <div class="boxform form-control">
                        ...
                        @foreach($regions as $region)
                            <?php $j++; ?>
                            <label class="">
                                <input name="region[]" id="region[{{$j}}]" value="{{$region['id']}}" type="checkbox" onchange="sendData('region='+this.value+'&on='+this.checked,'{{route ('ajax.bbordmapLocalite')}}','villeitem')"> {{$region['name']}}
                            </label><br>
                        @endforeach
                    </div>
                </div>
                <div class="col-sm-6">
                    <label class="section-name" for="villes">Ville</label><br>
                    <div class="boxform form-control">
                        <div id="villeitem"></div>
                    </div>
                </div>
            </div>
        </div>
