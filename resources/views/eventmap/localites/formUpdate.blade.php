            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                    <div class="all-form-element-inner">
                        <form class="form-group-inner" role="form" method="POST" action="{{ route('storeLocalites') }}">
                            @csrf
                            <input type="hidden" name="pid" value="{{ $localite->id }}">
                            <div class="col-sm-6 form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                <label for="name">Nom Localité</label>
                                <input id="name" placeholder="" type="text" class="form-control" name="name" value="{{ $localite->name ?? old('name') }}"  autofocus>
                                @if ($errors->has('name'))
                                ...
                                <div class="chosen-select-single mg-b-20">
                                    ...
                                    <select  class="form-control" name="parent" id="selectItem" tabindex="-1" onchange="sendData('localite='+this.value,'{{route('ajax.getFilsLocalite')}}','selectItem')">
                                        ...
                                        @foreach($localites as $r)
                                            @php($selected = $localite->parent === $r['id'] ? "selected" : "")
                            ...
                            <div class="col-sm-6 form-group{{ $errors->has('latitude') ? ' has-error' : '' }}">
                                ...
                                <input placeholder="" type="text" class="form-control" name="latitude" value="{{ $localite->latitude ?? old('latitude') }}"  autofocus id="latItem" onchange="sendData('lat='+this.value,'{{route('ajax.changerCoord','lat')}}', 'latDivItem')">
                            ...
                            <div class="col-sm-6 form-group{{ $errors->has('longitude') ? ' has-error' : '' }}">
                                ...
                                <input  placeholder="" type="text" class="form-control" name="longitude" value="{{ $localite->longitude ?? old('longitude') }}"  autofocus id="lngItem" onchange="sendData('lng='+this.value,'{{route('ajax.changerCoord','lng')}}', 'lngDivItem')">
                            ...
                            <div class="col-sm-12 form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                                ...
                                <textarea id="description" rows="10" placeholder="" type="text" class="form-control" name="description" >{{ $localite->description ?? old('description') }}</textarea>
